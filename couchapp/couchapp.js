...
ddoc.lists = {
    ...
    , atomfeed: function (){
        ...
        var date3339String = function (date){
            ...
            return  zeroPad(date.getUTCFullYear(), 4) + "-"
                    ...
                    + zeroPad(date.getUTCMilliseconds(), 3) + "-00:00";
        };

        var atomHeader = function (){
        ...
        var atomFooter = function (){
            return  "</feed>";
        };

        var atomFormatted = function (row){
            if (row.doc){
                return  [ "<entry>"
        ...
        var row;
        while(row = getRow()){
            var formatted = atomFormatted(row);
            send(formatted);
        }

        var footer = atomFooter();
        return footer;
    }
};
...
ddoc.validate_doc_update = function (newDoc, oldDoc, userCtx) {
    ...
    if (!newDoc._deleted && oldDoc && oldDoc.type === "buildset" && userCtx.roles.indexOf('builder') === -1 && userCtx.roles.indexOf('_admin') === -1){
